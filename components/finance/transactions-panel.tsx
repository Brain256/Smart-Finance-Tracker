...
export function TransactionsPanel({ accuracy, canDelete, expenses, financeTimezone, reviewThreshold, sortState, onCorrection, onSortStateChange, deleteError, deletingExpenseId, isDeletePending, onDelete }: TransactionsPanelProps) {
  ...
  return <div className="grid gap-5"><ClassificationAccuracyPanel accuracy={accuracy} /><section className="rounded-lg border border-[var(--border)] bg-white shadow-sm"><div className="flex items-center gap-3 border-b border-[var(--border)] p-4"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-slate-100 text-slate-700"><ReceiptText aria-hidden="true" className="h-4 w-4" /></div><div><h2 className="text-base font-semibold tracking-normal text-slate-950">Transactions</h2><p className="text-sm text-[var(--muted)]">{sortedExpenses.length} of {expenses.length} records sorted by {sortState.key}</p></div></div><FilterControls filters={filters} onChange={setFilters} onClear={() => setFilters(EMPTY_EXPENSE_FILTERS)} />{deleteError ? <div className="border-b border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800" role="alert">{deleteError}</div> : null}{sortedExpenses.length === 0 ? <p className="px-4 py-6 text-sm text-[var(--muted)]">{expenses.length === 0 ? "No transactions have been recorded yet." : "No transactions match the current search and filters."}</p> : <div className="min-w-0 max-w-full"><table className="block w-full min-w-0 text-left text-sm sm:table sm:table-fixed"><thead className="hidden bg-slate-50 text-xs uppercase text-slate-500 sm:table-header-group"><tr className="sm:table-row">{columns.map(([key, label]) => <th className="px-4 py-3 font-semibold" key={key}><button className="focus-ring inline-flex items-center gap-1 rounded-sm text-left hover:text-slate-900" onClick={() => onSortStateChange((current) => nextSortState(current, key))} type="button">{label}<SortIcon sortKey={key} sortState={sortState} /></button></th>)}<th className="w-52 px-4 py-3 font-semibold">Category editor</th>{canDelete ? <th className="w-32 px-4 py-3 text-right font-semibold">Actions</th> : null}</tr></thead><tbody className="block divide-y divide-[var(--border)] sm:table-row-group">{sortedExpenses.map((expense) => {
    const needsReview = isUnverified(expense, reviewThreshold);
    const confidenceUnavailable = expense.confidence == null;

    return <tr className="block border-b border-[var(--border)] p-3 hover:bg-slate-50 sm:table-row sm:border-b-0 sm:p-0" key={expense.id}><td className="block px-0 py-1 text-slate-700 sm:table-cell sm:px-4 sm:py-3">{formatDateTime(expense.timestamp)}</td><td className="block px-0 py-1 font-medium text-slate-950 sm:table-cell sm:px-4 sm:py-3"><div className="flex flex-wrap items-center gap-2"><span>{expense.merchantName}</span>{needsReview ? <button aria-label={`Unverified category for ${expense.merchantName}. Focus category editor`} className="focus-ring inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-900 hover:bg-amber-200" onClick={() => selectRefs.current[expense.id]?.focus()} type="button"><AlertTriangle aria-hidden="true" className="h-3 w-3" />Unverified</button> : confidenceUnavailable ? <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">Confidence unavailable</span> : null}</div></td><td className="block px-0 py-1 text-slate-700 sm:table-cell sm:px-4 sm:py-3">{expense.category}</td><td className="block px-0 py-1 font-semibold text-slate-950 sm:table-cell sm:px-4 sm:py-3">{formatCurrency(expense.amount)}</td><td className="block px-0 py-1 sm:table-cell sm:px-4 sm:py-3"><CategoryEditor expense={expense} onCorrection={onCorrection} selectRef={(element) => { selectRefs.current[expense.id] = element; }} /></td>{canDelete ? <td className="block px-0 py-1 text-right sm:table-cell sm:px-4 sm:py-3"><button aria-label={`Delete transaction from ${expense.merchantName}`} className="focus-ring inline-flex items-center gap-1.5 rounded-md px-2 py-1.5 text-xs font-semibold text-red-700 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50" disabled={isDeletePending} onClick={() => onDelete(expense)} type="button"><Trash2 aria-hidden="true" className="h-3.5 w-3.5" />{isDeletePending && deletingExpenseId === expense.id ? "Deleting..." : "Delete"}</button></td> : null}</tr>;
